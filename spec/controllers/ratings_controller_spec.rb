...
RSpec.describe RatingsController, type: :controller do
  ...
  let(:rating) { FactoryGirl.create(:rating, book_id: book.id, state: 'approved') }
  ...
  describe "GET #index" do
    ...
    it "receives where on Rating"\
      ...
      expect(Rating).
        to receive(:where).with(state: 'approved').and_return [rating]
    end
  ...
  #     after { get :edit, { id: rating.id.to_s } }
      
  #     it_behaves_like "customer authentication"
  #     it_behaves_like "rating setting"

  #     it "receives find on Rating with requested id" do
  #       expect(Rating).to receive(:find).with(rating.id.to_s).and_return(rating)
  #     end
  #   end 
