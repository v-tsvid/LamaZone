...
RSpec.describe "order_items/index", type: :view do
  ...
  it "displays order-items' prices" do
    @order_items.each do |item|
      expect(rendered).to have_content item.send(:price)
    end
  end    
  ...
  it "displays delete buttons for each order_item" do
    @order_items.each do |item|
      expect(rendered).to have_selector "input[type='submit'][id='book_#{item.book.id}_remove']"
    end
  end

  it "displays 'back to shopping' link" do
    expect(rendered).to have_link 'Back to shopping', books_path
  end
end
